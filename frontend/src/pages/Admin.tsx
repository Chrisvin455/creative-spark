...
export default function Admin() {
  ...
  const savePrompt = async () => {
    ...
    if (editingPrompt) {
      // NOTE: Update prompt needs an endpoint on the backend in real scenarios, assuming PUT /api/prompts/:id exists or skipping it for demo.
    } else {
      await fetchApi("/prompts", { method: 'POST', body: JSON.stringify({ text: promptText, category_id: promptCategory }) });
  ...
  const deletePrompt = async (id: string) => {
    // Requires DELETE /api/prompts/:id on backend
    toast({ title: "Prompt deleted (not implemented on backend)" });
    queryClient.invalidateQueries({ queryKey: ["admin-prompts"] });
  };

  const addCategory = async () => {
    if (!catName || !catSlug) return;
    await fetchApi("/categories", { method: 'POST', body: JSON.stringify({ name: catName, slug: catSlug, color_key: "fiction", icon: "BookOpen" }) });
  ...
  const deleteCategory = async (id: string) => {
    // Requires DELETE /api/categories/:id on backend
    toast({ title: "Category deleted (not implemented)" });
    queryClient.invalidateQueries({ queryKey: ["categories"] });
  };
